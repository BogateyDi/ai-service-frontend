...
interface AssistantViewProps {
  ...
  onToggleSetting: (setting: 'internetEnabled' | 'memoryEnabled') => void;
  onRequestReferralCode: () => void;
  favoriteServices: FavoriteService[];
  onAddFavorite: (service: FavoriteService) => void;
  onRemoveFavorite: (service: FavoriteService) => void;
  onNavigateToService: (service: FavoriteService) => void;
  onLogout: () => void;
  // Admin Mode Props
  ...
  setUserAccounts: React.Dispatch<React.SetStateAction<Map<string, UserAccount>>>;
}
...
const AdminDashboard: React.FC<{
  ...
  currentUserCode: string | null,
  onLogout: () => void
}> = ({ userAccounts, setUserAccounts, onExitAdminMode, currentUserCode, onLogout }) => {
    const [editGenerations, setEditGenerations] = useState<Record<string, string>>({});
    const [editLimits, setEditLimits] = useState<Record<string, string>>({});
    const [searchTerm, setSearchTerm] = useState('');
    type SortKey = 'code' | 'generations' | 'dataSize' | 'maxStorageSize';
    const [sortConfig, setSortConfig] = useState<{ key: SortKey; direction: 'ascending' | 'descending' }>({ key: 'code', direction: 'ascending' });
    ...
    const handleUpdateGenerations = (code: string) => {
    ...
    const handleUpdateLimit = (code: string) => {
    ...
    const handleToggleAssistant = (code: string, assistant: 'mirra' | 'dary') => {
        setUserAccounts(prev => {
            const newAccounts = new Map(prev);
            const account = newAccounts.get(code);
            if (account) {
    ...
    const StatCard: React.FC<{ title: string; value: string | number; icon: React.ReactNode; subtitle?: React.ReactNode; }> = ({ title, value, icon, subtitle }) => (
...
const ToggleButton: React.FC<{ isToggled: boolean; onClick: () => void; children: React.ReactNode; icon: React.ReactNode; }> = ({isToggled, onClick, children, icon}) => (
    <button
        onClick={onClick}
...
const FavoriteServiceButton: React.FC<{ service: FavoriteService; onNavigate: (service: FavoriteService) => void; onRemove: (service: FavoriteService) => void; }> = ({ service, onNavigate, onRemove }) => (
...
const MessageContentWithCopyButton: React.FC<{ text: string }> = ({ text }) => {
...
const AddFavoriteModal: React.FC<{ onClose: () => void; onAddFavorite: (service: FavoriteService) => void; }> = ({ onClose, onAddFavorite }) => {
    ...
    const [selectedDocType, setSelectedDocType] = useState<DocumentType>(STUDENT_DOC_TYPES_STANDARD[0]);
    const [age, setAge] = useState<number>(CHILDREN_AGES[6]); // default 12
                    ...
                    {audience === 'children' ? (
                        <>
                            ...
                            <div className="flex flex-col gap-2">
                                <VerticalSelector title="Написание работ" items={STUDENT_DOC_TYPES_STANDARD} selectedValue={selectedDocType} onSelect={(val) => setSelectedDocType(val as DocumentType)} />
                                <VerticalSelector title="Интерактив" items={STUDENT_DOC_TYPES_INTERACTIVE} selectedValue={selectedDocType} onSelect={(val) => setSelectedDocType(val as DocumentType)} />
                            </div>
                        </>
                    ) : (
                        <>
                            <VerticalSelector title="Категория" items={ADULT_CATEGORIES} selectedValue={adultCategory} onSelect={setAdultCategory} />
                            <VerticalSelector title="Формат" items={DOC_TYPES_BY_ADULT_CATEGORY[adultCategory] || []} selectedValue={selectedDocType} onSelect={(val) => setSelectedDocType(val as DocumentType)} />
                        </>
                    )}
                </div>
...
export const AssistantView: React.FC<AssistantViewProps> = ({
  ...
  currentUserCode, isAdminMode, onExitAdminMode, dataSize, generationHistory, userAccounts, setUserAccounts, onLogout
}) => {
  ...
  const findGenerationRecord = (id?: string): GenerationRecord | undefined => {
      if (!id) return undefined;
      return generationHistory.find(r => r.id === id);
  };

  const handleDownloadAttachment = (record: GenerationRecord) => {
